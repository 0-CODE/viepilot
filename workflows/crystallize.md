...
If notes.md contains any of: `## hw_intake`, `## hw_topology`, `## pin_map`, `## memory_layout`, `## protocols`, `## rtos_config`, `## embedded_toolchain`, `## power_budget`, `## safety_config`, `## secure_lifecycle`, `## test_strategy`, `## production`, `## success_metrics`, `## lab_equipment`:
  ...
  - **`## Test & Verification Strategy`** ← from `## test_strategy` (ENH-110)
    - Test layer table: Layer | Tooling | Runs on | Blocking gate | Notes
    - Host unit framework (Unity+CMock/Ceedling) + static analysis config (MISRA+cppcheck/clang-tidy)
    - HIL setup + test jig; firmware CI summary
    - Coverage target (gcov/lcov) + fault-injection scenarios (brownout/comm-error/WDT)
    - **Cross-link**: per-task verification is enforced by the ENH-108 2-tier contract
...
### Step 6 — CI/CD Version Bump Protocol injection (ENH-104)

After writing the base SYSTEM-RULES.md template content, check scan state:

IF `deployment_signals[]` is non-empty:
  Append the following section inside the `<versioning>` block of SYSTEM-RULES.md,
  immediately after the `### Version Bump Triggers` table:

  ```markdown
  ## Version Bump Protocol (CI/CD)
  <!-- generated by vp-crystallize from deployment_signals -->

  When running `/vp-evolve`, update version string in ALL of the following files:

  **Always update:**
  - `.viepilot/TRACKER.md`
  - `package.json` / `pom.xml` / primary version file
  - `CHANGELOG.md` — `[Unreleased]` section

  **Project CI/CD files (auto-detected at crystallize time):**
  {for each signal in deployment_signals[]:}
  - `{signal.file_path}` ← {signal.platform} — check for hardcoded `{current_version}` refs

  **Pre-commit audit command:**
  ```bash
  grep -rn "{old_version}" {space-separated list of CI/CD file paths}
  ```
  ```

  Platform-specific patterns to search per signal.platform:
  - `docker` / Dockerfile: `FROM {image}:{tag}` and `ARG VERSION=`
  - `github-actions`: `image:`, `APP_VERSION:`, version pins in `uses:`
  - `gitlab-ci`: `image:` and `variables:` VERSION entries
  - `docker-compose`: `image: {service}:{tag}` entries
  - `kubernetes` / `helm`: `image:` in manifests, `appVersion:` in Chart.yaml
  - other: literal `{current_version}` string occurrences

ELSE (deployment_signals empty or MISSING):
  Do NOT append the section. SYSTEM-RULES.md gets generic SemVer section only.
  (Print: `ℹ No CI/CD deployment signals detected — skipping Version Bump Protocol injection.`)
</step>
