...
- **Worktree isolation guidance (ENH-025)**: `workflows/autonomous.md` now offers isolated worktree execution for `L` / `XL` tasks (or explicit task overrides), and `templates/phase/TASK.md` seeds `worktree_isolation: "auto"` so new tasks can opt into `auto | always | never`.
