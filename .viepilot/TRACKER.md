...
- **Current Phase**: ~~150~~ — ✅ done (v3.12.1)
...
3.12.1 (released — DEBT-003 check-update OS session guard)
```
...
|------|----------|-----------|-------|
| 2026-07-15 | Phase 164 complete: ENH-115 npm publish/release **preflight gate** → v3.24.0 (9 resilient tests, full suite 2467/2467, full preflight exit 0). scripts/release-preflight.cjs (fail-closed: git clean/main/synced + npm-auth + version-consistency + stale-version-refs + not-published; content-only tier for verify:release, full tier for release path; --dry-run) + scripts/lib/version-refs.cjs (stale ref sweep — closes vp_evolve bump-gap; caught real README drift 3.21.0→3.24.0) + scripts/release.cjs orchestrator (preflight→tag→push→CI publish; --dry-run) + wired into verify:release/release-npm.yml + release-checklist upgraded. Reuses existing CI publish; bump via changelog-agent (no re-bump). ENH-115 resolved. tag vp-p164-complete | Codify manual npm release into a fail-closed repeatable gate | 164 |
| 2026-07-15 | Phase 164 planned: ENH-115 automated npm publish/release **preflight gate** → v3.24.0 (4 tasks). scripts/release-preflight.cjs (fail-closed: clean tree/main/synced + version-consistency + not-published + npm-auth) + scripts/lib/version-refs.cjs (stale version-ref sweep — closes vp_evolve bump-gap) + scripts/release.cjs orchestrator (--dry-run) + wire into verify:release/prepublishOnly/release-npm.yml. REUSES existing CI publish (no rebuild); delegates bump to changelog-agent (ENH-053, no re-bump) | Codify manual npm release into a fail-closed repeatable gate | 164 |
| 2026-07-14 | ENH-115 logged: automated npm publish/release workflow — guarded gates (clean tree/main/tests/version-consistency + stale-version grep) → npm publish + tag + tracker update, with --dry-run. Delegates bump to changelog-agent (no re-bump). Clarified from `/vp-request npm push` via AUQ = automate publish, not manual one-off | Codify manual npm releases into a fail-closed repeatable flow; addresses vp_evolve bump-gap | Backlog |
...
| 2026-07-01 | ENH-114 logged: vp-qa Live Role-Based Visual QA (`--live` mode) — idea→per-role feature matrix; seed users per role; agent-browser drives running app per role + screenshots; new `qa-design-reviewer` agent evaluates real screenshots vs vp-design ENH-102 aesthetic framework → UI-adjustment proposals; orchestrator emits BUG (functional)/ENH-FEAT (UI) → AUQ → /vp-evolve; reuse browser-audit-agent + browser-runner.cjs; graceful degrade to ENH-113 static if no agent-browser/runnable app. Effort L; recommend brainstorm before plan | User real-world prompt extracted into a live full-system QA phase; distinct from ENH-113 static gap analysis | Backlog |
...
| 2026-06-29 | **Milestone v3.2 COMPLETE** (phases 156–160, ENH-106..112 → v3.21.0). Phase 160: ENH-112 3-phase engagement template (Discovery→Bring-up Demo→Pilot) + `## success_metrics` + `## lab_equipment` probes; crystallize exports Success Metrics + Lab Equipment; synced ENH-071 page count 6→9; README badges synced (version 3.8.0→3.21.0, skills 21→22 +vp-qa). Embedded Architect pages 6→9; intake gaps #8/#10/#11/#12/#13 closed. Flagged: 19 pre-existing test failures + README narrative drift = follow-up DEBT | Close embedded milestone; gap #13 engagement motion from embedder.com research | 160 |
| 2026-06-29 | Phase 159 complete: ENH-111 Production & Manufacturing → v3.20.0 (12 tests). `🏭 Production & Manufacturing` brainstorm probe + `## production`; 9th architect page `production.html` (count 8→9); crystallize export `## Production & Manufacturing` → ARCHITECTURE.md (NOT PROJECT-CONTEXT.md as SPEC said — sibling consistency) cross-linked to ENH-109 keys | Gap #12 production firmware ≠ dev firmware; factory test/provisioning/calibration/serialization needed at intake | 159 |
| 2026-06-29 | Phase 158 complete: ENH-110 Embedded Testing & Verification → v3.19.0 (12 tests). `🧪 Testing & Verification` brainstorm probe + `## test_strategy`; 8th architect page `test-strategy.html` (count 7→8); crystallize export `## Test & Verification Strategy` cross-linked to ENH-108. Made phase157/158 page-count asserts resilient (`>=N`) | Gap #11 firmware test strategy needed at intake; count-bump broke phase157 hard-coded "7 pages" assert → fixed pattern | 158 |
...
|----|------|-------|----------|--------|
| ENH-115 | 🔧 | Automated npm publish/release workflow — preflight/test/version gates → npm publish + tag + tracker update, with --dry-run | medium | ✅ done (**3.24.0**, Phase 164) |
