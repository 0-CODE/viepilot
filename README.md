...
**Versioning:** Shield **1.9.3** is the **ViePilot framework SemVer** tracked in `.viepilot/TRACKER.md` and `CHANGELOG.md`. The npm `package.json` field `version` (**1.9.3**) is the Node package identifier for this repo; use the framework version for milestone releases and docs.
...
|-----------------|-----------------|
| Total LOC | **~27,546+** (`.md`, `.js`, `.cjs`, `.yml`, `.json`, `.sh`; không gồm `node_modules`) |
| Skills | **16** |
| Workflows | **12** |
| Templates | **17** (Project: 12, Phase: 5) |
| CLI Commands | **18** (`vp-tools` 17 subcommands + `viepilot` installer) |
| Tests | **299** (15 suites: unit + integration + AI compat + README metrics + UI direction + ENH contracts + scope policy + FEAT-009 + FEAT-010 + ENH-021 routing + FEAT-001 Claude Code docs + viepilot-info/update/install) |
| ViePilot phases (local `.viepilot`) | **34** phase cycles — **M1.29** FEAT-001 (**v1.9.3**); **M1.28** FEAT-010 (**v1.9.1**); patch **v1.9.2** (ENH-019~021); **M1.27** (**v1.9.0** FEAT-009); xem `CHANGELOG.md` |
...
| Test Files | 15 | Jest: 14 unit + 1 integration (contracts, installer, info/update, FEAT-009, FEAT-010, FEAT-001, ENH-021, scope policy, …) |
...
```
Tổng thể / Overall:  ████████████████████  ~98% ✅ Latest **v1.9.3** (FEAT-001: **Claude Code** setup doc); **v1.9.2** (ENH-019~021); **v1.9.1** FEAT-010; **v1.9.0** FEAT-009
```
...
| Tests (299) | ✅ Hoàn thiện | Unit, integration, AI compat, workflow contracts, installer, scope policy, FEAT-009/010/001, ENH-021, info/update |
