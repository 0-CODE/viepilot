...
function buildInstallPlan(packageRoot, envSource = process.env, opts = {}) {
  ...
  if (installClaudeSkills && claudeSkillsDir) {
    ...
    if (claudeViepilotDir) {
      ...
      for (const f of ['cli-shared.cjs', 'viepilot-info.cjs', 'viepilot-update.cjs', 'viepilot-install.cjs', 'viepilot-config.cjs']) {
  ...
  steps.push({
    ...
    // When autoYes, writes defaults without prompting
    autoYes: env.autoYes,
    home,
  });
