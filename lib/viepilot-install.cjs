...
function buildInstallPlan(packageRoot, envSource = process.env, opts = {}) {
  ...
  if (installClaudeSkills && claudeSkillsDir) {
    ...
    if (claudeViepilotDir) {
      ...
      // Copy package.json so vp-tools info resolves correct installed version
      ...
      // Rewrite template/workflow path references in mirrored workflow files (BUG-B)
      steps.push({
        kind: 'rewrite_paths_in_dir',
      ...
      // Rewrite template path references in mirrored template files (BUG-B)
      steps.push({
        kind: 'rewrite_paths_in_dir',
  ...
  // So `vp-tools info` resolves package root from ~/.cursor/viepilot (any cwd) — matches ~/.claude/viepilot/ (BUG-005 parity)
  steps.push({
    kind: 'copy_file',
    from: path.join(root, 'package.json'),
    to: path.join(viepilotDir, 'package.json'),
  });
