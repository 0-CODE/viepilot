...
const ADAPTER_CONTEXTS = {
  ...
  'zed': {
    ...
    tools: {
      ...
      glob:         'find_path',
      ls:           'list_directory',
      web_search:   'search_web',     // Zed Pro + Zed provider only
      web_fetch:    'fetch',
      notebook_read: null,
      notebook_edit: null,
      tool_search:  null,
      todo_read:    null,
      todo_write:   null,
      agent:        'spawn_agent',    // callable subagent; same tools as parent
      interactive:  null,             // no AskUserQuestion
    },
    interactive:  'text',
    subagent:     'single-level',     // spawn_agent is callable; vp-auto fan-out stays claude-code-only
    orchestration: {
      mode:            'sequential',  // vp-auto Agent-tool fan-out is claude-code specific
      parallel:        false,
      teams:           false,
      background:      false,
      model_override:  null,
    ...
    mcp: { supported: true, tool_limit: null },
    skill_path_project: '.agents/skills',
    skill_path_global:  '~/.agents/skills',
    agents_dir: null,
  },
};
...
function listAdapterIds() {
  return ['claude-code', 'cursor-agent', 'antigravity', 'codex', 'copilot', 'zed'];
}
...
function detectAdapter() {
  ...
  if (env.ZED_TERM || env.ZED_ENDPOINT) return 'zed';
  ...
  if (fs.existsSync(path.join(home, '.config', 'zed'))) return 'zed';
  if (fs.existsSync(path.join(home, '.zed'))) return 'zed';
